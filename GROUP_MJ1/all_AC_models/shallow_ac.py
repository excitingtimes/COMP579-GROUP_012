...
class Agent():
  ...
  def load_weights(self,location=''):
    if(len(location) >0):
      self.model.load_state_dict(torch.load(location,map_location=torch.device(self.device)))
...
class ActorCritic(nn.Module):
  ...
  def __init__(self, use_viz= True, use_scent= True, use_bin=True ):
    ...
    if self.use_viz:
      ...
      self.critic_viz_fwd = nn.Sequential(*[nn.Conv2d(in_channels=3, out_channels= 9,kernel_size= (3,3), stride=2 ),
                                            torch.nn.ReLU(),
                                            nn.Flatten(), 
                                            nn.Linear(7*7*9, 24), 
                                            nn.ReLU(),
      ...
      self.actor_viz_fwd = nn.Sequential(*[nn.Conv2d(in_channels=3, out_channels= 9,kernel_size= (3,3), stride=2 ),
                                            torch.nn.ReLU(),
                                            nn.Flatten(), 
                                            nn.Linear(7*7*9, 24), 
                                            nn.ReLU(),
    ...
    if self.use_bin:
      ...
      self.critic_bin_fwd = nn.Sequential(*[nn.Conv2d(in_channels=4, out_channels= 9,kernel_size= (3,3), stride=2),
                                            torch.nn.ReLU(), 
                                            nn.Flatten(), 
                                            nn.Linear(7*7*9, 24), 
                                            nn.ReLU(),
      ...
      self.actor_bin_fwd = nn.Sequential(*[nn.Conv2d(in_channels=4, out_channels= 9,kernel_size= (3,3), stride=2),
                                           torch.nn.ReLU(), 
                                           nn.Flatten(), 
                                           nn.Linear(7*7*9, 24), 
                                           nn.ReLU(),
