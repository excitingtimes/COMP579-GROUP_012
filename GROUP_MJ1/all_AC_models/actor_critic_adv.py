  ...
  def update(self, curr_obs, action, reward, next_obs, done, timestep):
    ...
    if(curr_obs is not None ):
      ...
      if((timestep % self.batch_size == 0 or done) and len(self.a)>3):
        ...
        #td target
        td_target= rewards[:-3,...] + self.gam * rewards[1:-2,...]  + self.gam **2 * rewards[2:-1,...] + self.gam **3 * rewards[3:,...] + self.gam **4 * v_stp1[3:,...]
        
        #td error
        delta =  (td_target -v_st[:-3,...])
        
        #logprob of actions taken
        log_probs = pi_st.gather(1,prev_actions[:-3,...] ).log()
        
        #update model
        self.model.zero_grad()
        loss = -log_probs * delta.detach() + self.critic_loss(td_target.detach(), v_st[:-3,...]) #add entropy for expl?
        loss.mean().backward()
        self.optim.step()   
