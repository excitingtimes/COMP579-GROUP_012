  ...
  def update(self, curr_obs, action, reward, next_obs, done, timestep):
    ...
    if(curr_obs is not None ):
      ...
      if((timestep % self.batch_size == 0 or done) and len(self.a)>3):
        ...
        #entropy
        entropy = -(pi_st * pi_st.log()).sum(1, keepdim=True)
        
        #update model
        self.model.zero_grad()
        loss = -log_probs * delta.detach() + self.critic_loss(td_target.detach(), v_st) +entropy
        loss.mean().backward()
        self.optim.step()   
...
class ActorCritic(nn.Module):
  ...
  def __init__(self, use_viz= True, use_scent= True, use_bin=True ):
    ...
    if self.use_bin:
      ...
      self.critic_bin_fwd = nn.Sequential(*[nn.Conv2d(in_channels=4, out_channels= 32,kernel_size= 1 ),
                                            torch.nn.ReLU(),
