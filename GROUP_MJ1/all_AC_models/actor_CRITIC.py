...
class Agent():
  ...
  def __init__(self, env_specs, use_bin=True, use_scent=True,
    ...
    self.model_a = Actor(use_bin=self.use_bin, use_scent=self.use_scent, use_viz=self.use_viz).float()
    self.model_c = Critic(use_bin=self.use_bin, use_scent=self.use_scent, use_viz=self.use_viz).float()
    #parallel computation across multiple gpus 
    self.multithread=multithread
    
    if(self.multithread):
      self.model_a= torch.nn.DataParallel(self.model_a)
    ...
    self.optim_a = torch.optim.Adam(self.model_a.parameters(),lr=0.01)
    self.optim_c = torch.optim.Adam(self.model_c.parameters(),lr=0.1)
  ...
  def load_weights(self,location=''):
    if(len(location) >0):
      self.model_a.load_state_dict(torch.load(location,map_location=torch.device(self.device)))
      self.model_c.load_state_dict(torch.load(location,map_location=torch.device(self.device)))
  ...
  def update(self, curr_obs, action, reward, next_obs, done, timestep):
    ...
    if(curr_obs is not None ):
      ...
      if((timestep % self.batch_size == 0 or done) and len(self.a)>3):
        ...
        pi_st = self.model_a(**prev_states)
        v_st  = self.model_c(**prev_states)
        
        #next state value estim
        v_stp1= self.model_c(**next_states)
        ...
        if(done or (len(self.store_model)>0 and (self.num_updates% self.store_freq)==0 )):
          ...
          if(self.multithread):
            torch.save(self.model_a.module.state_dict(), f"{self.store_model}/actor-{self.store_model}-mdl-it{self.num_updates}.pth")
            torch.save(self.model_c.module.state_dict(), f"{self.store_model}/critic-{self.store_model}-mdl-it{self.num_updates}.pth")
          else:
            torch.save(self.model_a.state_dict(), f"{self.store_model}/actor-{self.store_model}-mdl-it{self.num_updates}.pth")
            torch.save(self.model_c.state_dict(), f"{self.store_model}/critic-{self.store_model}-mdl-it{self.num_updates}.pth")
...
class Actor(nn.Module):
  '''This is a class of Actor that takes in a
  visual representation of the space and outputs
  an action to take.
  This actor can make use of any of the outputs of the environment'''
  ...
  def forward(self, x_viz=None, x_scent=None, x_bin=None):
    ...
    return self.actor_out(act_out)
  
class Critic(nn.Module):
  '''This is a class of Critic that takes in a
  ...
  def forward(self, x_viz=None, x_scent=None, x_bin=None):
    ...
    return self.critic_out(crit_out)