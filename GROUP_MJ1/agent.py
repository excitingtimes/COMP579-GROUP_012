...
class Agent():
  ...
  def __init__(self, env_specs, n=50, discount=0.85, learning_rate=0.0001, epsilon=1, epsilon_decay=0.9,
               markovian_window=3, buffer_size=5000, G_t_cutoff=0.1, n_epochs=100, model_save_path='GROUP_MJ1/weights.pt', initialize_weights=True,
  ...
  def load_weights(self, root_path):
    ...
    self.q_estimator.load_state_dict(torch.load(root_path + "weights.pt"))
  ...
  def update(self, curr_obs, action, reward, next_obs, done, timestep):
    # initialize q_estimator weights
    if timestep==0 and self.initialize_weights:
      self.load_weights('GROUP_MJ1/')
      print('uploaded')
    # identify when new episode begins
    if curr_obs is None:
      self.episode_start = timestep
    # decay epsilon and save model weights every episode
    if timestep % 5000 == 0:
      self.epsilon *= self.epsilon_decay
      print(self.epsilon)

      torch.save(self.q_estimator.state_dict(), self.model_save_path)
    ...
    if (timestep - self.episode_start) >= self.n and curr_obs is not None:
      ...
      # add to buffer
      if abs(G_t.item()) > self.G_t_cutoff:
      ...
      if len(self.scent_buffer) == self.buffer_size:
        print('TRAIN BUFFER')
